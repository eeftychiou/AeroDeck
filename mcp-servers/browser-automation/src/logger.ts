...
const defaultLogPath = path.resolve(process.cwd(), "browser-automation.log");
...
export function setupStdioProtection() {
  ...
  process.on("uncaughtException", (error: Error) => {
    logMessage("ERROR", "Uncaught Exception detected in Browser Automation process", {
      name: error.name,
  ...
  process.on("unhandledRejection", (reason: any) => {
    logMessage("ERROR", "Unhandled Promise Rejection detected in Browser Automation process", reason);
  });

  logMessage("INFO", "Browser Automation Stdio Guard initialized. All debug logs routed to stderr and " + logFilePath);
}
