...
AeroDeck is opinionated about worktree management — specific paths (`.worktrees/<branch>`), specific commands (`git worktree add`), specific cleanup (`git worktree remove`). Meanwhile, Claude Code, Codex App, Gemini CLI, and Cursor all provide native worktree support with their own paths, lifecycle management, and cleanup.
...
For harnesses without native support (Codex CLI, OpenCode, Copilot standalone), aerodeck fills a real gap. The skill shouldn't go away — it should get out of the way when native support exists.
...
- `.aerodeck-session.json` metadata tracking (interesting PR #997 idea, not needed for v1)
...
Whoever creates the worktree owns its cleanup. If the harness created it, aerodeck doesn't touch it. If aerodeck created it (via git fallback), aerodeck cleans it up. The heuristic: if the worktree lives under `.worktrees/` or `~/.config/aerodeck/worktrees/`, aerodeck owns it. Anything else (`.claude/worktrees/`, `~/.codex/worktrees/`, `.gemini/worktrees/`) belongs to the harness.
...
2. Check for existing `~/.config/aerodeck/worktrees/<project>/` directory — if found, use it (backward compatibility with legacy global path).
...
No interactive directory selection prompt. The global path (`~/.config/aerodeck/worktrees/`) is no longer offered as a choice to new users, but existing worktrees at that location are detected and used for backward compatibility.
...
git worktree remove "$WORKTREE_PATH"  # only if aerodeck owns it
...
**Option 4 (Discard):** Require typed "discard" confirmation. Then remove worktree (if aerodeck owns it), force-delete branch.
...
if worktree path is under .worktrees/ or ~/.config/aerodeck/worktrees/:
    # AeroDeck created it — we own cleanup
...
The `.worktrees/` or `~/.config/aerodeck/worktrees/` = ours, anything else = hands off` heuristic works for every current harness. If a future harness adopts `.worktrees/` as its convention, we'd have a false positive (aerodeck tries to clean up a harness-owned worktree). Similarly, if a user manually runs `git worktree add .worktrees/experiment` without aerodeck, we'd incorrectly claim ownership. Both are low risk — every harness uses branded paths, and manual `.worktrees/` creation is unlikely — but worth noting.
