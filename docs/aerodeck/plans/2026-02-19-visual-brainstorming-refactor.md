...
> **For agentic workers:** REQUIRED: Use aerodeck:subagent-driven-development (if subagents available) or aerodeck:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.
...
**Spec:** `docs/aerodeck/specs/2026-02-19-visual-brainstorming-refactor-design.md`
...
```bash
cd /Users/drewritter/prime-rad/aerodeck && node tests/brainstorm-server/server.test.js
```
...
Search the codebase:
```bash
grep -r "wait-for-feedback" /Users/drewritter/prime-rad/aerodeck/ --include="*.js" --include="*.md" --include="*.sh" --include="*.json"
```
...
```bash
cd /Users/drewritter/prime-rad/aerodeck && lib/brainstorm-server/start-server.sh --project-dir /tmp/brainstorm-smoke-test
```
...
```bash
grep -r "wait-for-feedback\|sendToClaude\|feedback-footer\|send-to-claude\|TaskOutput.*block.*true" /Users/drewritter/prime-rad/aerodeck/ --include="*.js" --include="*.md" --include="*.sh" --include="*.html" | grep -v node_modules | grep -v RELEASE-NOTES | grep -v "\.md:.*spec\|plan"
```
