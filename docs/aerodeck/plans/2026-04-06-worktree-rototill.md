...
**Goal:** Make aerodeck defer to native harness worktree systems when available, fall back to manual git worktrees when not, and fix three known finishing bugs.
...
**Spec:** `docs/aerodeck/specs/2026-04-06-worktree-rototill-design.md`
...
2. **Check for existing global directory:**
   ...
   ls -d ~/.config/aerodeck/worktrees/$project 2>/dev/null
   ```
...
Global directories (`~/.config/aerodeck/worktrees/`) need no verification.
...
# For global: path="~/.config/aerodeck/worktrees/$project/$BRANCH_NAME"
...
**If worktree path is under `.worktrees/` or `~/.config/aerodeck/worktrees/`:** AeroDeck created this worktree — we own cleanup.
...
- **Fix:** Only clean up worktrees under `.worktrees/` or `~/.config/aerodeck/worktrees/`
...
```markdown
- **aerodeck:using-git-worktrees** - REQUIRED: Set up isolated workspace before starting
```

to:

```markdown
- **aerodeck:using-git-worktrees** - Ensures isolated workspace (creates one or verifies existing)
```
