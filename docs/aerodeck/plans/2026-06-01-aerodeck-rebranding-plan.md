...
**Goal:** Rebrand the aerodeck project fully to "AeroDeck" for the Google Antigravity 2.0 platform, updating all metadata, skill structures, references, and tests, and resetting the repository for a fresh GitHub push.
...
- [ ] **Step 2: Verify current state lacks criteria**
    Verify the existing files still use "aerodeck" by checking content.
    Expected: File content contains `"name": "aerodeck"`.
...
### Task 2: Rename OpenCode plugin and files (.opencode/plugins/aerodeck.js -> .opencode/plugins/aerodeck.js)

**Targets:**
- Rename/Move: `.opencode/plugins/aerodeck.js` -> `.opencode/plugins/aerodeck.js`
- Modify: `.opencode/plugins/aerodeck.js`

- [ ] **Step 1: Write/Define success criteria**
    Explicit criteria:
    *   File `.opencode/plugins/aerodeck.js` does not exist.
    *   File `.opencode/plugins/aerodeck.js` exists.
    *   Internal content in `aerodeck.js` references `aerodeck` paths, plugins, and configs.

- [ ] **Step 2: Verify current state lacks criteria**
    Check for the existence of `.opencode/plugins/aerodeck.js` and missing `.opencode/plugins/aerodeck.js`.
    Expected: File `aerodeck.js` exists.

- [ ] **Step 3: Perform minimal implementation / worker action**
    Move the file:
    ```bash
    mv .opencode/plugins/aerodeck.js .opencode/plugins/aerodeck.js
    ```
    Edit `.opencode/plugins/aerodeck.js` to replace all references to `aerodeck` with `aerodeck` and update `AeroDeckPlugin` to `AeroDeckPlugin`:
...
- [ ] **Step 4: Verify state passes criteria**
    Verify the file `.opencode/plugins/aerodeck.js` exists and contains `AeroDeckPlugin` instead of `AeroDeckPlugin`.
    Expected: PASS

- [ ] **Step 5: Commit/Save**
    ```bash
    git add .opencode/plugins/aerodeck.js
    git rm .opencode/plugins/aerodeck.js
    git commit -m "chore: rename plugin entrypoint to aerodeck.js"
    ```

---

### Task 3: Rebrand and rename using-aerodeck skill (skills/using-aerodeck -> skills/using-aerodeck)

**Targets:**
- Rename/Move: `skills/using-aerodeck/` -> `skills/using-aerodeck/`
...
- [ ] **Step 1: Write/Define success criteria**
    Explicit criteria:
    *   `skills/using-aerodeck/` is completely renamed to `skills/using-aerodeck/`.
...
- [ ] **Step 2: Verify current state lacks criteria**
    Check for path `skills/using-aerodeck/SKILL.md`.
    Expected: Paths exist.

- [ ] **Step 3: Perform minimal implementation / worker action**
    Rename the directory:
    ```bash
    mv skills/using-aerodeck skills/using-aerodeck
    ```
    ...
    - Rename `using-aerodeck` or references where appropriate.
    Update `GEMINI.md`:
    ```text
...
- [ ] **Step 5: Commit/Save**
    ...
    git rm -r skills/using-aerodeck
    git commit -m "chore: rename using-aerodeck skill to using-aerodeck"
    ```
...
- Modify: `README.md`
- Modify: `CLAUDE.md`
- Modify: All `skills/*/SKILL.md` files (specifically updating `aerodeck:` skill prefix to `aerodeck:`)

- [ ] **Step 1: Write/Define success criteria**
    Explicit criteria:
    *   Grep search for `aerodeck:` or `aerodeck` shows no remaining skill or framework prefixes.
    *   `README.md` and `CLAUDE.md` fully rebranded.

- [ ] **Step 2: Verify current state lacks criteria**
    Verify that existing skill files mention `aerodeck:`.
    Expected: True.

- [ ] **Step 3: Perform minimal implementation / worker action**
    Iterate through all `skills/*/SKILL.md` files and replace `aerodeck:` with `aerodeck:`.
    Also update references in `README.md` and `CLAUDE.md` to rebrand "AeroDeck" to "AeroDeck".

- [ ] **Step 4: Verify state passes criteria**
    Run search:
    ```bash
    grep -r "aerodeck:" skills/
    ```
...
- [ ] **Step 1: Write/Define success criteria**
    Explicit criteria:
    *   Test scripts reference `using-aerodeck` instead of `using-aerodeck`.
    *   Static mappings verify `skills/using-aerodeck/references/antigravity-tools.md`.

- [ ] **Step 2: Verify current state lacks criteria**
    Verify existing test scripts contain `"using-aerodeck"`.
    Expected: True.

- [ ] **Step 3: Perform minimal implementation / worker action**
    Update the paths in `tests/antigravity/` files to replace `using-aerodeck` references with `using-aerodeck`.
    Update test expectations to match `aerodeck` rather than `aerodeck`.
